 ...
 *
 * Flow: Identify wallet → LOCK → Validate → Update + Ledger → RELEASE
 */
...
import { validateCurrencyActive } from "../currencies/currency.config.js";
...
export async function processDeposit(params: DepositParams): Promise<DepositResult> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const wallet = await ensureWallet(tx, params.userId, params.currency);
    ...
    console.log("WALLET BEFORE:", { wallet_id: locked.id, balance: currentBalance });
    console.log("WALLET AFTER:", { wallet_id: locked.id, balance: newBalance, amount: netAmount });
