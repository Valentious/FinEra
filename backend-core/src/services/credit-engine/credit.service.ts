...
import { InterestRateEngine } from "./domain/interest-rate.service.js";
...
export async function calculateCreditLimit(
  ...
  const user = await prisma.user.findUnique({
    where: { id: userId },
    include: {
      wallets: currency ? { where: { currencyCode: currency } } : true,
      loans: true,
    },
  });
  ...
  const loansInScope = currency
    ? user.loans.filter((l) => l.currency === currency)
    : user.loans;

  const existingLoansTotal = loansInScope
  ...
  if (currency) {
    ...
    walletBalances[currency] = w ? Number(w.balance) : 0;
  } else {
