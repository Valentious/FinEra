/**
 * FinEra KYC-level identity input formats (frontend).
 * Mirrors backend `kyc-identity-formats.ts` — keep rules in sync.
 */
...
/** Strip bidi overrides, zero-width, control chars (keep ordinary space). */
export function stripKycInvisible(input: string): string {
  const s = input.normalize("NFKC");
  return s.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F\u200B-\u200F\u202A-\u202E\u2060-\u2064\uFEFF]/g, "");
}

/** Extract up to 12 meaningful chars for ZW national ID: 2 digits, 7 digits, 1 letter, 2 digits. */
...
/** Display with single spaces between segments (auto-mask while typing). */
...
/** Student / exam number: 1 letter + 8 digits + 1 letter, no spaces. */
...
/** Staff / alumni institutional ID: letter + 5–19 alphanumeric (total 7–20). */
...
/**
 * Residential address: at least 3 commas → 4 non-empty segments (street, house number, city, town).
 * Combines line1 and line2 with a comma boundary.
 */
